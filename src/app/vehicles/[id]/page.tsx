...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (rocket) {
    const title = `${rocket.name} | OpStellar`;
    const description = rocket.description || `${rocket.name} vehicle specs and launch history on OpStellar.`;
    const image = (rocket as unknown as { flickr_images?: string[] }).flickr_images?.[0];
    return {
      title,
      description,
      openGraph: { title, description, ...(image ? { images: [{ url: image, width: 1200, height: 630, alt: rocket.name }] } : { images: ["/brand/opstellar-og.png"] }) },
      twitter: { card: "summary_large_image", title, description, images: [image || "/brand/opstellar-og.png"] },
    };
  }
  return { title: `Vehicle ${params.id} | OpStellar`, description: "Vehicle details on OpStellar." };
}
